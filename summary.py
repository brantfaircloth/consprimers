...
def worker(input, minConsensusLength, minAlignLength, metadataKey):
    ...
    conn = MySQLdb.connect(user='python', passwd='', db='birdcons')
    cur = conn.cursor()
    file = open(input,'rU')
...
def main():
    ...
    if n_procs > 1:
        ...
        try:
            ...
            while files:
                if len(threads) < n_procs:
                    p = multiprocessing.Process(target=worker, args=(files.next(), minConsensusLength, minAlignLength, metadataKey))
                    p.start()
                    threads.append(p)
                else:
                    for p in threads:
        ...
        try:
            while files:
                worker(files.next(), minConsensusLength, minAlignLength, metadataKey)
        except StopIteration:
            pass
